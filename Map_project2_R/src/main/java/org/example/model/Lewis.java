...
public class Lewis extends Personaggio implements Serializable {
    ...
    public String interpretaComandoDaGUI(String comando) {
        ...
        switch (verbo) {
            case "leggi":
                if (oggetto.contains("documento") && inventario.contieneOggetto("documento")) {
            ...
            case "salva":
                int slotDisponibile = GameSaver.trovaPrimoSlotLibero();

                // Se non c'è spazio libero, chiedi se sovrascrivere lo slot 1
                if (slotDisponibile == -1) {
                    int scelta = JOptionPane.showConfirmDialog(null,
                            "Tutti gli slot sono pieni. Vuoi sovrascrivere lo slot 1?",
                            "Slot pieni", JOptionPane.YES_NO_OPTION);
                    if (scelta == JOptionPane.YES_OPTION) {
                        slotDisponibile = 1;
                    } else {
                        return "> Salvataggio annullato.";
                    }
                }

                StatoGioco stato = new StatoGioco(this, inventario, mappa, storyEngine);
                if (GameSaver.salvaPartita(stato, slotDisponibile)) {
                    return "> Partita salvata nello slot " + slotDisponibile + ".";
                } else {
    ...
    private void salvaGioco() {
        StatoGioco stato = new StatoGioco(this, inventario, mappa, storyEngine);
        boolean successo = GameSaver.salvaPartita(stato, 1); // oppure chiedi lo slot
        if (successo) {
    ...
    private String entraInStanza(Stanza stanza) {
        ...
        if (stanza.getOggetto() == null && !stanza.getNome().equalsIgnoreCase("scale") && !stanza.getNome().equalsIgnoreCase("corridoio")) {

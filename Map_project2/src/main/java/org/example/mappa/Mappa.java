package org.example.mappa;

public class  Mappa {
    private final int larghezza;
    private final int altezza;
    private final Stanza[][] stanze;

    public Mappa(int larghezza, int altezza) {
        this.larghezza = larghezza;
        this.altezza = altezza;
        this.stanze = new Stanza[altezza][larghezza];
        inizializza();
    }

    private void inizializza() {
        // Primo piano
        stanze[5][6] = new Stanza("Salotto", true, false, true, false, true, false, true, true);
        stanze[2][6] = new Stanza("Cucina", true, false, true, false, false, true, false,false);
        stanze[5][10] = new Stanza("Bagno", false, false, true, false, false,false,false,true);
        stanze[2][3] = new Stanza("Stanza Segreta", true, true, false, false,false,true,false,false); // chiusa inizialmente
        stanze[5][2] = new Stanza("Scale", true, false, false, true,true,false,true,false);

        // Secondo piano (DA RIVEDERE)
        stanze[3][9] = new Stanza("Stanza Matrimoniale", true, false, true, false, false, false, false, true);
        stanze[0][1] = new Stanza("Balcone", true, false, false, true, false, true, false, false);
        stanze[3][1] = new Stanza("Stanza dei Bambini", true, false, true, false, true, false, true, false);
        stanze[0][9] = new Stanza("Stanza degli ospiti", true, false, false, false, false, false, false, true);
        stanze[6][5] = new Stanza("Scale", true, false, false, false, true, true, false, false);
        stanze[3][5] = new Stanza("Corridoio", true, false, false, false, true, true, true, true);
        stanze[0][5] = new Stanza("Corridoio", true, false, false, false, false, true, true, false);
    }

    public boolean isValida(Posizione p) {
        int x = p.getX();
        int y = p.getY();
        boolean dentroMappa = y >= 0 && y < stanze.length && x >= 0 && x < stanze[0].length;
        boolean esisteStanza = dentroMappa && stanze[y][x] != null;

        System.out.println("Verifica posizione valida -> x=" + x + ", y=" + y +
                " | dentroMappa: " + dentroMappa + ", esisteStanza: " + esisteStanza);

        return esisteStanza;
    }

    public Stanza getStanza(Posizione posizione) {
        return stanze[posizione.getY()][posizione.getX()];
    }

    public String mostra(Posizione pos) {
        StringBuilder sb = new StringBuilder();
        for (int y = 0; y < altezza; y++) {
            for (int x = 0; x < larghezza; x++) {
                if (pos.getX() == x && pos.getY() == y) {
                    sb.append("P ");
                } else if (stanze[y][x] != null) {
                    sb.append("S ");
                } else {
                    sb.append(". ");
                }
            }
            sb.append("\n");
        }
        return sb.toString();
    }
    public int getLarghezza() {
        return larghezza;
    }

    public int getAltezza() {
        return altezza;
    }
}

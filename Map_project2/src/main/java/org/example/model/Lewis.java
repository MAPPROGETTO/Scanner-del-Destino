...
public class Lewis extends Personaggio {
    ...
    private List<Scena> scene; // Aggiungi una lista di scene per il dialogo
    ...
    public Lewis(Mappa mappa, GestoreInputGUI inputGUI, Inventario inventario, List<Scena> scene) {
        super("Lewis", "Un giovane inventore orfano...", false);
        this.mappa = mappa;
        this.inputGUI = inputGUI;
        this.inventario = inventario;
        this.posizione = new Posizione(6, 5); // centro mappa 5x5
        this.scene = scene; // Aggiungi le scene per il dialogo
    }
    ...
    public String interpretaComandoDaGUI(String comando) {
        ...
        switch (verbo) {
            case "prendi":
                ...
                String oggettoNellaStanza = stanza.getOggetto();
                if (oggettoNellaStanza == null || !oggettoNellaStanza.equalsIgnoreCase(oggetto)) {
                    ...
                    // Avvia la scena con nome "nomeStanza_2" se esiste
                    String nomeScena = stanza.getNome() + "_2";
                    for (Scena scena : scene) {
                        if (scena.getNome().equalsIgnoreCase(nomeScena)) {
                            messaggio += "\n" + scena.mostra();
                            break;
                        }
                    }
    ...
    private String entraInStanza(Stanza stanza) {
        String dialogo = stanza != null ? stanza.entra() : null;
        if (stanza != null && stanza.getNome().equals("Casa Abbandonata")) {
            dialogo += "\n" + this.scene.get(5).mostra();
        }
        return dialogo;
    }
}